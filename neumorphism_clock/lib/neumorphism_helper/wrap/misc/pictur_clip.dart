...
import 'package:digital_clock/neumorphism_helper/wrap/misc/image_clip.dart';
...
class PicturClipWidget extends StatelessWidget {
  PicturClipWidget(
      ...
      : assert(imageParams.clipPath.where((v) => v.dx > 1 || v.dy > 1).length == 0,
            "clipPath has values greater than 1"),
        super();
  ...
  PicturClipWidget.network(
      {GlobalKey key,
      String urlImage,
        BackgroundParams backgroundParams, ImageParams imageParams,})
      : this(
            imageKey: key,
  ...
  PicturClipWidget.localFile({GlobalKey key,
...
enum ImageType {
  CLIPPED, CLIPPED_FLOATING, ORIGINAL_IMAGE
}
...
double normalizeToModule1(double normalizedPosition) {
  if (normalizedPosition == null || normalizedPosition < 0
...
class FractionedBackground extends StatelessWidget {
  ...
  FractionedBackground(double normalizedPosition)
      :_normalizedPosition = normalizeToModule1(
      normalizedPosition); //TODO add parameters to change height and position
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        return Stack(children: <Widget>[Align(
